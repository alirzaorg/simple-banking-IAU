...
def add_message(messages: MessageQueue, account_number: str) -> MessageQueue:
    print("ما خیلی مفتخر میشویم تا بازخورد شما را بدانیم!!!")
    ...
    while failed_attempt:
        ...
        if not utils.is_valid_message(message):
            failed_attempt -= 1
            print("پیامتان طولانی است لطفا پیامی کوتاه تر انتخاب نمایید!!!")
            print("تلاش های باقی مانده : %d!!!" % failed_attempt)
        else:
            break
...
def read_message(messages: MessageQueue) -> Optional[MessageQueue]:
    print("دیدن بازخورد کاربران برای بهبود سیستم")
    if not messages.size:
        print("پیامی برای خواندن موجود نیست")
        utils.proceed_next()
        return messages
